...
| 确认没问题，记录这次变更 | `git add .` → `git commit -m "说明"` | 拍快照——这就是代码变更的一次精确锚定 |
...
**Git 不是“开发者的工具”，是服务端部署范式能运转的前提**——没有版本管理，生产变更的输入只是模糊的临时代码；有了 Git，每次发布都绑定在 commit hash 锚定的精确快照上。
...
- **Git 锁定了代码版本，但锁不住运行环境**。如何把“编译好的产物 + 运行环境本身”一起打包？这就是 第 05 篇（Docker 容器化） 登场要解决的根本问题。

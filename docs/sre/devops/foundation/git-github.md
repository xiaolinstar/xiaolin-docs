...
上一篇（03 服务端应用部署）把构建动作搬到了服务器：Spring Boot 要 `mvn package`，Flask 要 `pip install`，服务器上多出源码、依赖、构建产物三套东西。这里藏着一个真问题：**服务器上正在跑的这次构建，对应哪一份代码？**

如果回答不了这个问题，回退、排查、复现都会变成「靠记忆猜」。**痛点三：没有版本追踪**——手动备份会被跳过、没有上下文、回退要重做一遍——在服务端部署场景下变得更加危险。

这个痛点在 $f$ 里的体现是：**$x$（变更内容）没有锚点。** 你改了什么、什么时候改的、改坏了回哪个版本、服务器上那次构建对应哪次提交——这些 $f$ 的输入信息散落在操作记忆里，没有被固化下来。

这一篇的解法是**版本管理**。

## 版本管理：03 篇痛点三的解法

03 篇提到服务器构建时手动备份 `dist-20260713` 的三个断点，Git 一次性补完：
...
1. **生成密钥对**
   ...
   # 私钥：~/.ssh/id_ed25519（本地保管，绝不外传）
   # 公钥：~/.ssh/id_ed25519.pub（交给 GitHub）
   ```

   提示 passphrase 时可以直接回车跳过，或设置一个口令加强安全。

2. **把公钥交给 GitHub**：复制 `id_ed25519.pub` 的全部内容 → GitHub → Settings → SSH and GPG keys → **New SSH key** → 粘贴保存。

3. **把 remote 协议从 HTTPS 切到 SSH**

   ```bash
   git remote set-url origin git@github.com:你的用户名/你的仓库名.git
   ```

之后 `git push` / `git pull` / `git clone git@github.com:...` 都走 SSH 通道，全程免密。

> 私钥（`id_ed25519`）**绝对不能提交到仓库，也不能粘贴到任何聊天 / 笔记工具里**——丢了等于把 GitHub 访问权送人。建议 `chmod 600 ~/.ssh/id_ed25519` 限制权限；CI 服务器推荐用 **Deploy Key**（仓库级别只读 / 读写）或专用 token，而不是复用个人私钥。

> 仓库里不要提交密码、Token、私钥。用 `.gitignore` 忽略 `.env` 等敏感文件；万一推上去了，应立刻轮换密钥，而不是只删文件再推一次。

## Commit 信息：开发者最该写好的那一行

`git init` / `add` / `push` 都会变成肌肉记忆，**唯独 commit 信息写什么，过去时常是一件伤脑筋的事**——三个月后排查问题的人、接手代码的新同事、回溯事故的运维，先看的都是这一行。`update` / `fix bug` / `tmp` 这类含糊写法，让历史变成"考古谜题"。
...
过去写 commit 信息是开发者的"软素质"——靠自觉、靠 code review 提醒、靠团队规范约束。**AI Agent 时代则升级为"靠工程化治理"**：自动提交拦截、Conventional Commits 强校验（commitlint）、自动信息填充、commit 质量门禁（pre-commit + 服务端分支保护）等议题已经远超本文范围。

详见 [《AI Agent 时代下重新审视 Git》](../../../ai/theory/git-in-ai-agent-era.md)，本文不展开。
...
之前 $x$ 是一个模糊的「当前代码」——你改了什么、什么时候改的，没有锚点。有了 Git 之后：

- **$x$ 变得精确**：每次 `git commit` 就是一次 $x$ 的快照，有 hash、有时间、有说明
- **$f$ 的输入可追溯**：部署 = `git checkout <commit>` → 构建 → 部署，每一步都有据可查
- **回退变成重跑**：出问题 → `git revert` → 重新跑一遍 $f$ → 回到上一个稳定状态

$X$（规范集合）的边界也因此更清晰——$X$ 内的变更都有 commit 记录，突破 $X$ 的变更（换构建工具、改部署方式）也有对应的 commit 可以追溯。

**Git 不是「开发者的工具」，是 $f$ 能运转的前提**——没有版本管理，$x$ 就是一个模糊的「当前代码」；有了 Git，$x$ 是一次精确的 commit。
...
- **平台中立**：同一份 `.md` 可以渲染到 VitePress / GitHub Pages / GitBook / Notion / 语雀 / 公众号编辑器，不被任何厂商锁定
- **AI 友好**：LLM 天然生成与解析 Markdown，AI 写的文档可直接 commit，CI 可自动校验格式

这正是 Git × Markdown 流行的根本原因：**写作者只需要"想清楚写什么 + 用 Git 管理变更"，下游的渲染、分发、协同全部交给生态**。运营 / 产品 / 学生 / 研究者把 Markdown 当成"轻量 Office"、把 Git 当成"轻量 SharePoint"——门槛低、收益大、跨平台。
...
**一句话总结**：**一切即代码，代码即可版本管理**。把"贴在 Word 里的截图 / 嵌入对象"换成"写在 Markdown 里的公式、Mermaid、IaC 配置"，就同时获得 diff 可读、版本可追、轻松渲染、**极其方便迁移**——一份 `.md` 走到任何平台（VitePress / GitBook / Notion / 自建站）都能渲染，不用担心厂商锁定。

本站文档里所有的公式、架构图、流水线 YAML 示例，都是这一思路的产物。

### AI 原生范式：从 Git 引用后开始接管日常工作

到这里，Git 和 Markdown 的基本能力已经齐备。**从下一篇（05 Docker）开始，就让 Git × Markdown 接手绝大多数日常工作**——文档、流水线、基础设施配置都建立在"代码 + 版本管理"这个底座之上。后续篇章会展开：

- **基础设施即代码（IaC）**：05 篇 Docker / Docker Compose、06 篇流水线基础、07 篇 GitHub Actions 都会落到这一层——服务器配置不再"贴控制台截图"，而是写在仓库里的 YAML / Dockerfile
- **AI 原生协作**：在 AI Agent 时代，"Git + Markdown + 声明式配置"是人和 AI 协作的**公共语言**——AI 写 Markdown 文档、提交 commit，人类审阅、合并；CI 自动校验格式、自动构建、自动部署。本文本身也是 "AI 原生" 的实践产物——本站所有内容由 Claude（AI）+ Markdown + Git + VitePress 协作产出。

本站的所有文档（包括本文）都是 Markdown + VitePress + Git 仓库的组合。

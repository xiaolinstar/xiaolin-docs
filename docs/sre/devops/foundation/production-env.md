...
要让网站上线，生产环境必须跨越两个最低门槛：**7 × 24 小时运行** 与 **具备公网 IP**。对于个人而言，最快捷、低成本的达成方案就是租用一台「云服务器」。
...
不过发布链本身可以**模板化**：构建 → 上传 → 落盘 → 验证。这就是**流水线**（Pipeline）的雏形——也是本系列给「运维工作」下定义的入口。
...
**手动备份解决不了版本管理。** 它只是把「覆盖」变成了「覆盖前先复制一份」，但「历史可追溯」、「变更可对比」、「回退可一键」这些能力，手动方式都给不了。这些不是备份的问题，是**版本管理**的问题——下一步我们会看到，Git 天然就有这套能力。
